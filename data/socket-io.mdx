...
The [data service](https://data.veranta.xyz/) serves the pair catalog
(`GET /v2/trading`) and broadcasts every change over Socket.IO as `RES:DATA`.
This is the live feed behind `client.pair_data_stream()` and the Veranta
...
| --- | --- | --- |
| Mainnet | `https://data.veranta.xyz` | `https://prod-api.veranta.xyz/data` |
| Testnet | `https://testnet-data.veranta.xyz` | `https://staging-api.veranta.xyz/data` |
...
```bash
curl https://data.veranta.xyz/v2/trading
```
...
`https://prod-api.veranta.xyz/data` as the URL; that joins namespace
...
const socket = io("https://data.veranta.xyz", {
  path: "/socket.io",
...
```javascript
const socket = io("https://prod-api.veranta.xyz", {
  path: "/data/socket.io",
  transports: ["websocket", "polling"],
  reconnection: true,
});
...
let snapshot = await fetch("https://data.veranta.xyz/v2/trading").then((r) => r.json());
...
socket.on("connect", async () => {
  snapshot = await fetch("https://data.veranta.xyz/v2/trading").then((r) => r.json());
});
```
...
Both SDKs wrap this stream. The callback receives the raw `RES:DATA` payload. The SDK does not merge it into `client.markets`, whose snapshot is polled over HTTP with a five-second cache. Treat the stream as its own live copy, or force a snapshot refresh when you need the typed pair models updated.
...
await stream.run((diff) => {
  console.log("pairs updated:", Object.keys(diff.pairInfos ?? {}));
});                                // blocks; stream.stop() to end
```

</CodeGroup>

Runnable scripts: `examples/17_streams.py` in the Python repo and `examples/17_streams.ts` in the TypeScript repo.
