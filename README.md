...
- **Published mirror**: the `avantis-python-sdk` repo (root), which the Mintlify deployment at https://sdk.veranta.com is connected to. Sync by copying this directory over its root; the trees are kept byte-identical (the mirror only adds a LICENSE) so `diff -r` verifies it:
