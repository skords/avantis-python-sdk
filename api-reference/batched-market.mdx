...
| Network | Base URL |
| --- | --- |
| Mainnet | `https://prod-api.veranta.xyz/batched-market` |
| Testnet | `https://staging-api.veranta.xyz/batched-market` |
...
# High level: signs the intent plus the optional EIP-7702 leg, streams, settles.
receipt = await client.trade.market_open("ETH/USD", "long", collateral=100, leverage=10)

# Same, but watch the journey live while the SDK settles.
receipt = await client.trade.market_open(
    "ETH/USD", "long", collateral=100, leverage=10,
    on_event=lambda ev: print("order", ev.type, ev.seq, ev.data),
)

# Market-maker fast path: local intent, intent-only POST, settle off the hot path.
builder = await client.local_intents()
payload = builder.open_trade_coin(...)
...
import { AggregatorOrderType } from "veranta-sdk";

// High level: signs the intent plus the optional EIP-7702 leg, streams, settles.
let receipt = await client.trade.marketOpen("ETH/USD", "long", { collateral: 100, leverage: 10 });

// Same, but watch the journey live while the SDK settles.
receipt = await client.trade.marketOpen("ETH/USD", "long", {
  collateral: 100, leverage: 10,
  onEvent: (ev) => console.log("order", ev.type, ev.seq, ev.data),
});

// Market-maker fast path: local intent, intent-only POST, settle off the hot path.
const builder = await client.localIntents();
const payload = builder.openTradeCoin({ /* ... */ });
receipt = await client.engine.submitIntentBatch(
  payload, AggregatorOrderType.MARKET_OPEN_WITH_COIN_EXPOSURE, { wait: false },
);
const outcome = await client.engine.batchedMarket.wait(receipt.trackingId!);  // replay by seq
```

</CodeGroup>

The event hook is available on every layer of the path: the trade methods, `submit_intent_batch` / `submitIntentBatch`, and the batched-market client's `execute` and `wait`. See [Follow the order lifecycle](/trading/market-orders#follow-the-order-lifecycle) and the [market-maker fast path](/advanced/mm-fast-path).
