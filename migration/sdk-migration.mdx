...
description: "Method-by-method mapping from the v1 TraderClient to the v2 AsyncVeranta client."
---

SDK 2.0 is a ground-up rewrite for Veranta v2 and **the 0.x/1.x API is
...
The package name is unchanged (`veranta-sdk` on PyPI); 2.0.0 is the
v2 release. Pinning the old client is only a stopgap; Veranta v1 is
superseded on-chain, so the 0.x SDK no longer trades:

```bash
pip install "avantis-trader-sdk<2"   # v1 (legacy)
pip install "veranta-sdk>=2"  # v2
```
...
| Client | `TraderClient(provider_url)` | `AsyncVeranta()`, configured from `VERANTA_*` env vars (sync facade: `Veranta()`) |
| Executing an order | `build_*_tx(...)` -> `sign_and_get_receipt(...)` | one method call returns an `ExecutionReceipt` |
| Gas | You hold ETH and pay gas | Gasless by default (relayer). `VERANTA_EXECUTION=direct` restores self-broadcasting |
...
| Errors | printed / raw contract reverts | typed exceptions from `veranta_sdk.errors` |
...
```bash
export VERANTA_PRIVATE_KEY=0x...      # API key (recommended) or trader key
export VERANTA_TRADER_ADDRESS=0x...   # your wallet, only when using an API key
```
...
1. **Key type.** The closest thing to 0.x is using your trader key directly
   (set `VERANTA_PRIVATE_KEY` to it, leave `VERANTA_TRADER_ADDRESS` unset).
   The recommended setup is an **API key**: a delegate key created with the
   [Veranta API Key Generator](https://delegate.veranta.xyz/)
...
2. **Execution route.** The default **relayer** route is gasless (signed
   intents, no ETH, no node). `VERANTA_EXECUTION=direct` with
   `VERANTA_RPC_URL` reproduces 0.x behavior: the SDK signs EIP-1559
...
(`pip install "veranta-sdk[kms]"`), passed to the client instead of
a private key.
